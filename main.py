...
app = FastAPI(
    ...
    version="1.3.4", # İstatistik endpointleri eklendi
...
async def update_table_status(masa_id: str, islem: str = "Erişim"):
    ...
    try:
        ...
        """, {"masa_id": masa_id, "son_erisim": now, "islem": islem}) # datetime objesi direkt
...
class AktifMasaOzet(BaseModel): # Bu model zaten vardı, `/admin/aktif-masa-tutarlari` için kullanılacak
    masa_id: str
    odenmemis_tutar: float
    aktif_siparis_sayisi: int
    siparis_detaylari: Optional[List[Dict]] = None # Frontend bunu kullanmıyorsa kaldırılabilir
...
# --- YENİ EKLENEN İSTATİSTİK MODELLERİ ---
...
class GunlukIstatistik(IstatistikBase):
    tarih: str # YYYY-MM-DD formatında
...
# AktifMasaOzet zaten vardı, /admin/aktif-masa-tutarlari için onu kullanacağız
# class AktifMasaTutarOzet(BaseModel):
#     masa_id: str
#     aktif_siparis_sayisi: int
#     odenmemis_tutar: float

# --- Korunan Endpoint Örnekleri ---
...
@app.get("/aktif-masalar/ws-count", tags=["Admin İşlemleri"]) # Etiket güncellendi
...
# --- YENİ EKLENEN İSTATİSTİK ENDPOINT'LERİ ---
...
async def get_gunluk_istatistik(
    ...
    try:
        ...
        # PostgreSQL'de zaman filtrelemesi için:
        # WHERE zaman >= :baslangic AND zaman < :bitis
        query = """
            SELECT sepet, durum FROM siparisler
            WHERE zaman >= :baslangic AND zaman < :bitis AND durum = 'odendi'
        """
        # Veritabanından zaman stringleri veya datetime nesneleri gelebilir.
        # Karşılaştırma için datetime nesnelerini kullanmak daha güvenlidir.
        # `databases` kütüphanesi tz-aware datetime'ları destekler.
        ...
        # Tüm 'odendi' siparişlerini say (eğer farklı bir sipariş sayısı tanımı isteniyorsa güncellenmeli)
        # Örnek olarak, tüm siparişleri alıp sonra filtreleyebiliriz veya sadece 'odendi' olanları sayabiliriz.
        # Şimdilik 'odendi' olan sipariş sayısını alıyoruz.
        # Eğer tüm sipariş sayısı (iptaller hariç) istenirse, sorgu ve mantık değişmeli.
        # Bu sorgu sadece 'odendi' siparişleri getirdiği için, siparis_sayisi bunların sayısı olacak.
...
async def get_aylik_istatistik(
    ...
    try:
        ...
        # Bir sonraki ayın ilk gününü bulmak için:
        if target_ay == 12:
...
async def get_yillik_aylik_kirilim(
    ...
    try:
        ...
        # PostgreSQL için zaman extract etme: EXTRACT(YEAR FROM zaman), EXTRACT(MONTH FROM zaman)
        # Veya DATE_TRUNC('month', zaman)
        # Bu sorgu tüm 'odendi' siparişlerini çeker ve Python'da gruplar.
        # Daha büyük veri setlerinde doğrudan SQL'de gruplamak daha performanslı olabilir.
        query = """
            SELECT sepet, zaman FROM siparisler
            WHERE durum = 'odendi' AND EXTRACT(YEAR FROM zaman AT TIME ZONE 'Europe/Istanbul') = :yil
        """ # AT TIME ZONE 'Europe/Istanbul' TR_TZ için. Veya zamanları UTC saklayıp UTC çalışmak.
            # Eğer zaman sütunu zaten TIMESTAMPTZ ise, direkt EXTRACT yeterli olabilir.
            # En sağlıklısı, veritabanına UTC kaydetmek ve sorgularken TR_TZ'ye göre dönüştürmek veya
            # uygulama katmanında UTC çalışıp sadece sunumda TR_TZ'ye çevirmek.
            # Şimdiki DDL'de TIMESTAMPTZ kullandık, bu iyi.
        
        # TR_TZ'yi PostgreSQL'in anlayacağı bir string'e çevirelim veya direkt UTC çalışalım.
        # En temizi, zamanları UTC olarak saklamak ve sorgularda UTC kullanmak.
        # Sonra gerekirse Python'da TR_TZ'ye çevirmek.
        # Mevcut durumda zamanlar TR_TZ ile kaydediliyor gibi duruyor (datetime.now(TR_TZ)).
        # Bu durumda EXTRACT(YEAR FROM zaman) yeterli olabilir eğer PostgreSQL session timezone TR ise.
        # Ya da zamanları UTC'ye çevirip UTC yılını almak.
        # Şimdilik basitçe yıl filtresiyle tüm siparişleri çekip Python'da işleyeceğiz.
        # Veritabanı zaman sütununun TIMESTAMPTZ olduğu varsayımıyla:
        yil_baslangic = datetime(target_yil, 1, 1, tzinfo=dt_timezone.utc) # Yıl başlangıcı UTC
        yil_bitis = datetime(target_yil + 1, 1, 1, tzinfo=dt_timezone.utc)   # Yıl sonu UTC

        # Eğer zaman sütunu 'Europe/Istanbul' (TR_TZ) olarak saklanıyorsa:
        ...
        """
        # `zaman` sütununun tipine göre (TIMESTAMPTZ mi, yoksa naive mi) `baslangic` ve `bitis`i ayarlamalı.
        # `init_db`de `TIMESTAMP WITH TIME ZONE` kullandık, bu yüzden tz-aware datetime göndermek doğru.
        odenen_siparisler_yil = await db.fetch_all(query_all_year, {"baslangic": yil_baslangic_tr, "bitis": yil_bitis_tr})

        aylik_kirilim: Dict[str, Dict[str, Any]] = {}

        for siparis in odenen_siparisler_yil:
            siparis_zamani = siparis["zaman"]
            # Gelen zaman tz-aware ise direkt kullan, değilse TR_TZ ata (eğer DB'de naive ise)
            if siparis_zamani.tzinfo is None:
                siparis_zamani = siparis_zamani.replace(tzinfo=TR_TZ) # Veya UTC varsay
            else: # Zaten tz-aware ise TR_TZ'ye çevir (eğer farklıysa)
                siparis_zamani = siparis_zamani.astimezone(TR_TZ)

            ay_key = siparis_zamani.strftime("%Y-%m")

            if ay_key not in aylik_kirilim:
                aylik_kirilim[ay_key] = {"toplam_gelir": 0.0, "satilan_urun_adedi": 0}
            ...
            try:
                ...
                for item in sepet_items:
                    adet = item.get("adet", 0)
                    fiyat = item.get("fiyat", 0.0)
                    aylik_kirilim[ay_key]["toplam_gelir"] += adet * fiyat
                    aylik_kirilim[ay_key]["satilan_urun_adedi"] += adet
            except json.JSONDecodeError:
        ...
        response_data = {
            ...
            for key, value in aylik_kirilim.items()
        }
...
async def get_en_cok_satilan_urunler(
    ...
    try:
        ...
        urun_sayaclari = VeliCounter() # Alias kullandık
...
async def get_aktif_masa_tutarlari(
    ...
    try:
        # Ödenmemiş durumlar: bekliyor, hazirlaniyor, hazir
        ...
        # `wherein` operatörü veya sorguyu formatlama gerekebilir.
        # Şimdilik basit bir IN ile deniyoruz, `databases` bunu handle etmeli.
        # Doğrusu: `durum = ANY(:status_list)` ve values={"status_list": odenmemis_durumlar}
        # Veya `IN ({placeholders})` ve formatlama.
        # `databases` kütüphanesi `IN` için `wherein` kullanır.
        # Örnek: `query = QueryBuilder.from_("siparisler").select("masa", "sepet").where(Column("durum").wherein(odenmemis_durumlar))`
        # Ama `databases` ile direkt string sorgu yazıyorsak:
        status_placeholders = ", ".join([f"'{s}'" for s in odenmemis_durumlar]) # Stringleri tırnak içine al
        query_str = f"SELECT masa, sepet FROM siparisler WHERE durum IN ({status_placeholders})"
        
        # fetch_all ile direkt string sorgu
        aktif_siparisler = await db.fetch_all(query_str)
...
async def cancel_order_by_customer_endpoint(
    ...
    if isinstance(olusturma_zamani, str): # String ise datetime'a çevir
        try:
            olusturma_zamani_dt = datetime.fromisoformat(olusturma_zamani)
        except ValueError:
             olusturma_zamani_dt = datetime.strptime(olusturma_zamani, "%Y-%m-%d %H:%M:%S").replace(tzinfo=TR_TZ) # Fallback
    else: # Zaten datetime ise
        olusturma_zamani_dt = olusturma_zamani

    if olusturma_zamani_dt.tzinfo is None: # Naive ise TR_TZ ata
...
SISTEM_MESAJI_ICERIK_TEMPLATE = (
    ...
    "Fıstık Kafe, ikinci nesil kahveler, özel çaylar, sıcak ve soğuk içecekler ile lezzetli atıştırmalıklar (kek, kurabiye vb.) sunan bir mekandır; KAFEDE YEMEK (pizza, kebap, ana yemek vb.) SERVİSİ BULUNMAMAKTADIR. " # KAFE KONSEPTİ EKLENDİ
    ...
    '  "toplam_tutar": 0.0,\n'
    '  "musteri_notu": "",\n'
    '  "konusma_metni": "Elbette! Çilekli ve soğuk bir şeyler arıyorsunuz. Menümüzdeki çilekli soğuk içeceklerden size örneğin Çilekli Milkshake\'i (eğer menümüzde varsa ve stoktaysa) önerebilirim. Ya da dilerseniz diğer soğuk içecek seçeneklerimize birlikte bakalım. Ne dersiniz?"\n' # STOK KONTROLÜ VURGUSU
    "}}\n\n"
    ...
    '  "sepet": [\n'
    '    {{\n'
    '      "urun": "Çilekli Milkshake",\n' # Bu ürünün {menu_prompt_data}'da olması varsayılıyor
    '      "adet": 1,\n'
    '      "fiyat": 25.0,\n' # Fiyat {menu_prompt_data}'dan gelmeli
    '      "kategori": "Soğuk İçecekler",\n' # Kategori {menu_prompt_data}'dan gelmeli
    '      "musteri_notu": ""\n'
    '    }}\n'
    '  ],\n'
    '  "toplam_tutar": 25.0,\n'
    ...
    '  "toplam_tutar": 0.0,\n'
    '  "musteri_notu": "",\n'
    '  "konusma_metni": "Fıstık Kafe\'de Adana dürüm gibi yemek servisimiz bulunmuyor, üzgünüz. Size bunun yerine özel harman bir filtre kahve veya serinletici bir ice latte hazırlamamı ister misiniz?"\n' # KONUŞMA METNİ GÜNCELLENDİ
    "}}\n\n"
    ...
    '  "toplam_tutar": 0.0,\n'
    '  "musteri_notu": "",\n'
    '  "konusma_metni": "Tabii, hemen Fıstık Kafe menümüzü sizinle paylaşıyorum: [AI BURADA {menu_prompt_data}\'dan ALDIĞI BİLGİLERLE KATEGORİLERE GÖRE MENÜ ÖZETİ SUNAR, YEMEK OLMADIĞINI VURGULARAK İÇECEK VE ATIŞTIRMALIKLARI ÖNE ÇIKARIR] ... Özellikle denemek istediğiniz bir kahve, çay veya atıştırmalık var mı?"\n' # KONUŞMA METNİ GÜNCELLENDİ
    "}}\n\n"
...
async def update_system_prompt():
    ...
    try:
    ...
    except KeyError as ke: # Özellikle KeyError'u yakala
        logger.error(f"❌ Sistem mesajı güncellenirken KeyError oluştu: {ke}. Şablonda eksik/yanlış anahtar olabilir.", exc_info=True)
        # Fallback, ama bu da aynı hatayı verebilir eğer şablon bozuksa.
        # Bu yüzden fallback'i daha güvenli hale getirelim.
        try:
        ...
        except Exception as fallback_e: # Fallback de hata verirse
            logger.error(f"❌ Fallback sistem mesajı oluşturulurken de hata oluştu: {fallback_e}", exc_info=True)
            # En kötü durum senaryosu: Basit, hatasız bir prompt.
...
async def add_menu_item_endpoint(
    ...
    try:
        async with menu_db.transaction():
            ...
            category_id_row = await menu_db.fetch_one("SELECT id FROM kategoriler WHERE LOWER(isim) = LOWER(:isim)", {"isim": item_data.kategori}) # Case-insensitive kontrol
...
async def delete_menu_item_endpoint(
    ...
    try:
        async with menu_db.transaction():
            item_to_delete = await menu_db.fetch_one("SELECT id FROM menu WHERE LOWER(ad) = LOWER(:ad)", {"ad": urun_adi}) # Case-insensitive kontrol
...
async def get_payable_orders_endpoint(
    ...
    try:
        ...
        if durum:
            ...
            # PostgreSQL'de IN için: durum = ANY(:status_list) ve status_list bir array olmalı.
            # Ya da IN ({placeholders}) ve formatlama. `databases` bunu yönetir.
            query_str = base_query + "durum IN :statuses ORDER BY zaman ASC"
            values["statuses"] = tuple(valid_statuses) # tuple olarak gönderilmeli IN için
